...
// - Use the calibration constants of U1 and D1 to compute 
//   the calibration coefficients that become the EPICS variables 
//   hac_bcm_A1 and hac_bcm_A2, which is applied to hac_bcm_dvm1_read and hac_bcm_dvm2_read 
//   to produce hac_bcm_dvm1_current and hac_bcm_dvm2_current 
// - These calibration coefficients are in units of uA/V  
...
int bcmCalibrateEPICS(const char *confPath){
   ...
   const int N = 2; 
   std::string bcmVar[N] = {"hac_bcm_dvm1_read","hac_bcm_dvm2_read"};
   ...
   for(int i=0;i<N;i++){
      ...
      for(int j=0;j<npar;j++) myFit[i]->SetParameter(j,0);
      // set titles 
      Title      = Form("%s",bcmVar[i].c_str());
      xAxisTitle = Form("Unser Current [#muA]");
      yAxisTitle = Form("%s [V]",bcmVar[i].c_str());
      // plot data 
      c1->cd(i+1); 
   ...
   sprintf(outpath,"%s/result_epics.csv",data_dir); 
