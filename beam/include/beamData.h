...
typedef struct beamData {
   ...
   double bpmA_rawcur_1;          // beam position current 1 (upstream)       
   double bpmA_rawcur_2;          // beam position current 2 (upstream)         
   double bpmA_rawcur_3;          // beam position current 3 (upstream)         
   double bpmA_rawcur_4;          // beam position current 4 (upstream)         
   double bpmB_rawcur_1;          // beam position current 1 (downstream)        
   double bpmB_rawcur_2;          // beam position current 2 (downstream)   
   double bpmB_rawcur_3;          // beam position current 3 (downstream)   
   double bpmB_rawcur_4;          // beam position current 4 (downstream)   
   ...
   int Print(){
      std::cout << Form("event %d, ",event) 
                ...
                << Form("BPMB y = %.3lf, ",bpmB_y) << std::endl;
      return 0; 
   } 
} beamData_t;   
